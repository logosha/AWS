...
namespace ACX
{
    public class BithumbClient : IBrokerClient
    {
        ...
        private const string rest_domain = "https://api.bithumb.com/public";

        private IDictionary<string, Subscribe> SubscribeList = new Dictionary<string, Subscribe>();

        private string _name = "Bithumb";
        ...
        public BithumbClient(IBrokerApplication client, ILogger logger)
        {
        ...
        public async Task Subscribe(Instrument instr, SubscriptionModel model)
        {
            ...
            var symbol = Utilities.BithumbSymbol(instr);
            using (await _lock.LockAsync())
            {
