...
namespace Kraken
{
    public class KrakenClient : IBrokerClient
    {
        ...
        private const string rest_domain = "https://api.kraken.com/0/public";

        private IDictionary<string, Subscribe> SubscribeList = new Dictionary<string, Subscribe>();

        private string _name = "Kraken";
        ...
        public async Task Subscribe(Instrument instr, SubscriptionModel model)
        {
            ...
            var symbol = Utilities.KrakenSymbol(instr);
            using (await _lock.LockAsync())
            {
