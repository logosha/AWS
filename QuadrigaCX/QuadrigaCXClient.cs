...
{
    public class QuadrigaCXClient : IBrokerClient
    {
        ...
        private const string rest_domain = "https://api.quadrigacx.com/v2";

        private IDictionary<string, Subscribe> SubscribeList = new Dictionary<string, Subscribe>();

        private string _name = "QuadrigaCX";

        public QuadrigaCXClient(IBrokerApplication client, ILogger logger)
        {
        ...
        public async Task<MarketBook?> GetTicker(Instrument instr)
        {
            ...
            string response = await REST.SendRequest(rest_domain + "/ticker?book=" + instr.First + "_" + instr.Second, "GET");
            ...
            result.ServerTime = new DateTime(1970, 1, 1, 0, 0, 0, 0).AddSeconds(Convert.ToDouble(tickersResponse.timestamp));
            result.LastPrice = Convert.ToDouble(tickersResponse.last);
            result.LastSize = Convert.ToDouble(tickersResponse.volume);
        ...
        public async Task Subscribe(Instrument instr, SubscriptionModel model)
        {
            ...
            var symbol = Utilities.QuadrigaCXSymbol(instr);
